...
from odfMiner.codeUpgrader import upgradeCode
import numpy as np
import pandas as pd
...
if aux:
    ...
    os.execv(sys.executable.replace("python.exe", "pdfMiner\explorePDF.py"))
...
class explorePDF():
        ...
        while count < len(array) - 1:
            ...
            while count < len(array) - 1 and aux == True:
                ...
                if len(array[count]) > 4:
                    ...
                    if array[count][len(array[count]) - 1] == '.' or array[count][len(array[count]) - 2] == '.' \
                            or array[count][len(array[count]) - 3] == '.' or array[count][len(array[count]) - 1] == ':' \
                            or array[count][len(array[count]) - 2] == ':' or array[count][len(array[count]) - 3] == ':':
                        i = i + 1
                        aux = False
        return paragraphs
    ...
    def getStructure(self, pdf_reader, fileName):
        ...
        while aux1 and n < 10:
            aux2 = True; m=-1;
            while aux2 and m<20:
                paragraphs1 = pdf_reader.loadPage(np.random.randint(0, pdf_reader.pageCount)-1) \
                    .getText("text")
                self.paragraphs1 = self.clarify(paragraphs1)
                paragraphs2 = pdf_reader.loadPage(np.random.randint(0, pdf_reader.pageCount)-1) \
                    .getText("text")
